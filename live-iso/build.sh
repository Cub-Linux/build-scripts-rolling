...
iso_publisher="Norcux OS Team"
iso_application="$os_name"
...
while getopts 'N:V:R:L:P:A:D:w:o:g:vh' arg; do
    case "${arg}" in
        N) iso_name="${OPTARG}" ;;
...
make_basefs() {
    mkarchiso-lts ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" init
    mkarchiso-lts ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode amd-ucode memtest86+ mkinitcpio-nfs-utils nbd zsh efitools" install
}

# Additional packages (airootfs)
make_packages() {
    mkarchiso-lts ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.x86_64)" install
}
...
make_setup_mkinitcpio() {
    ...
    ARCHISO_GNUPG_FD=${gpg_key:+17} mkarchiso-lts ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r 'mkinitcpio -c /etc/mkinitcpio-archiso.conf -k /boot/vmlinuz-linux-lts -g /boot/archiso.img' run
    if [[ ${gpg_key} ]]; then
      exec 17<&-
    fi
}
...
make_customize_airootfs() {
    ...
    sed -i "/PRETTY_NAME=/c\PRETTY_NAME=\"${os_name} ${iso_version//_/ }\"" ${work_dir}/x86_64/airootfs/etc/os-release
    sed -i "/VERSION=/c\VERSION=\"${iso_version//_/ }\"" ${work_dir}/x86_64/airootfs/etc/os-release
    sed -i "/Version=/c\Version=${iso_version//_/ }" ${work_dir}/x86_64/airootfs/etc/xdg/kcm-about-distrorc
    
    cp ${script_path}/pacman.conf ${work_dir}/x86_64/airootfs/etc/

    mkarchiso-lts ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/customize_airootfs.sh' run
...
make_boot() {
    ...
    cp ${work_dir}/x86_64/airootfs/boot/vmlinuz-linux-lts ${work_dir}/iso/${install_dir}/boot/x86_64/vmlinuz-lts
}
...
make_efiboot() {
    ...
    cp ${work_dir}/iso/${install_dir}/boot/x86_64/vmlinuz-lts ${work_dir}/efiboot/EFI/archiso/vmlinuz-lts.efi
...
make_prepare() {
    ...
    mkarchiso-lts ${verbose} -w "${work_dir}" -D "${install_dir}" pkglist
    mkarchiso-lts ${verbose} -w "${work_dir}" -D "${install_dir}" ${gpg_key:+-g ${gpg_key}} prepare
...
# Build ISO
make_iso() {
    mkarchiso-lts ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -P "${iso_publisher}" -A "${iso_application}" -o "${out_dir}" iso "${iso_label}-x86_64.iso"
}
...
cp ${script_path}/mkarchiso-lts /bin/
